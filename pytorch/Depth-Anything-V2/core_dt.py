...
def parse_args():
    ...
    parser.add_argument('--video-path', type=str, default='../../dataset/custom/video/machine/0_te3.mp4')
...
def calculate_proximity(objects, base_distance_threshold, depth_threshold):
    ...
    for i, obj1 in enumerate(objects):
        for j, obj2 in enumerate(objects[i + 1:], start=i + 1):
            ...
            # 근접성 판단 및 person과 machine/forklift 조합 확인
...
def process_frame(frame, yolo_model_1, yolo_model_2, yolo_model_3, depth_model, args, frame_number):
    results_1 = yolo_model_1(frame, conf=0.4, iou=0.1, verbose=False) # 지게차
    results_2 = yolo_model_2(frame, conf=0.4, iou=0.1, verbose=False) # 사람
    results_3 = yolo_model_3(frame, conf=0.4, iou=0.1, verbose=False) # 기계
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if depth_value is not None:
                    ...
                    print(f"프레임 {frame_number}: {label}에 대해 유효한 깊이 값을 찾을 수 없습니다.")
                    continue
    ...
    for obj1, obj2, distance_2d in proximity_pairs:
        ...
        print(f"프레임 {frame_number}: {obj1['label']}와 {obj2['label']}가 가까이 있습니다. 거리: {distance_2d:.1f}px")

    return frame, depth_color
...
def main(args):
    ...
    while video.isOpened():
        ...
        if frame_count % args.process_every_n_frames == 0:
            ...
            processed_frame, depth_map = process_frame(frame, yolo_model_1, yolo_model_2, yolo_model_3,
